    ...
    for top_k in "${top_ks[@]}"; do
      for rep in "${reps[@]}"; do
        for agent in "${agents[@]}"; do
          ...
          output_file="${RESULT_DIR}/${agent}_T${temp}_P${top_p}_K${top_k}_R${rep}_reverse_responses.json"

          echo "🔄 Running agent: $agent | T=$temp P=$top_p K=$top_k R=$rep"
          CUDA_VISIBLE_DEVICES=0,1,2,5,6,7 accelerate launch --main_process_port 0 --num_processes 1 "$SCRIPT" \
...
for merged_file in "${RESULT_DIR}"/test_responses_T*.json; do
  ...
  CUDA_VISIBLE_DEVICES=0,1,2,5,6,7 accelerate launch --main_process_port 0 --num_processes 1 /playpen-ssd/smerrill/llm_decisions/pairwise_comparison_completion.py \
    --data_file "$merged_file" \
    --overwrite \
    --max_responses "$MAX_RESPONSES"

  echo "Judging: $merged_file"
  CUDA_VISIBLE_DEVICES=0,1,2,5,6,7 accelerate launch --main_process_port 0 --num_processes 1 /playpen-ssd/smerrill/llm_decisions/judge_completion_responses.py \
    --data_file "$merged_file" \
    --overwrite \
    --max_responses "$MAX_RESPONSES"
